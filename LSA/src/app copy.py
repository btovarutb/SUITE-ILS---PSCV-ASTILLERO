...
CORS(app,
     ...
     resources={r"/*": {"origins": ["http://localhost:8010", "http://localhost:8080"]}},
...
def index():
    ...
    if request.method == 'POST':
        ...
        app.logger.info(datosPuertoBase)
        ...
        app.logger.info(f"Session después del GET: {dict(session)}")
        ...
        if desde_ils:
            app.logger.info("se va a cargar index.html")
            ...
            app.logger.info("Template renderizado correctamente")
            return template
        ...
        # Si tenemos buque_id pero no es desde_ils, redirigir a la vista de desglose
...
def registro_generalidades():
    ...
    if request.method == 'POST':
        try:
            ...
            nombre_equipo = nombre_equipo.strip() if nombre_equipo else "Sin nombre"

            fecha = request.form.get('fecha')
            fecha = fecha if fecha else "1970-01-01"

            fiabilidad_equipo = request.form.get('fiabilidad_equipo')
            fiabilidad_equipo = float(fiabilidad_equipo) if fiabilidad_equipo and fiabilidad_equipo.strip() else 0.0

            if mec and mec.split()[-1].isdigit():
                GRES = int(mec.split()[-1])
            else:
                GRES = 0  # Valor por defecto


            criticidad_equipo = request.form.get('criticidad_equipo')
            criticidad_equipo = int(criticidad_equipo) if criticidad_equipo and criticidad_equipo.strip() else 1

            marca = request.form.get('marca')
            marca = marca.strip() if marca else "Genérica"

            modelo = request.form.get('modelo')
            modelo = modelo.strip() if modelo else "N/A"

            peso_seco = request.form.get('peso_seco')
            peso_seco = float(peso_seco) if peso_seco and peso_seco.strip() else 0.0  # Por defecto 0.0
            ...
            dimensiones = dimensiones.strip() if dimensiones else "Desconocido"

            id_personal = request.form.get('responsable')
            id_personal = int(id_personal) if id_personal and id_personal.strip() else 1  # Por defecto 1 (usuario genérico)

            id_sistema = request.form.get('sistema')
            id_sistema = int(id_sistema) if id_sistema and id_sistema.strip() else 1

            id_equipo = request.form.get('equipo')
            id_equipo = int(id_equipo) if id_equipo and id_equipo.strip() else 1

            procedimiento_arranque = request.form.get('procedimiento_arranque')
            procedimiento_arranque = procedimiento_arranque.strip() if procedimiento_arranque else "No definido"

            procedimiento_parada = request.form.get('procedimiento_parada')
            procedimiento_parada = procedimiento_parada.strip() if procedimiento_parada else "No definido"

            id_buque = buque_id or 1  # <- Mueve esto antes de usarla
            id_sistema_ils = sistema_id_ils or 1

            id_subsistema = request.form.get('subsistema')
            id_subsistema = int(id_subsistema) if id_subsistema and id_subsistema.strip() else None

            cj = generar_codigo_jerarquico(id_subsistema, id_buque) if id_subsistema else "0000X"

            # Manejo de archivos (si no hay imagen, se manda None)
            ...
            app.logger.info(f"BUQUE: {id_buque}, Sistema {id_sistema_ils}")
            ...
            insertar_equipo_info(
                ...
                id_sistema_ils, id_buque, cj, id_subsistema  # ← Añadido al final
            )
...
@app.before_request
def before_request():
    app.logger.info("se ejecuto before_request")
    ...
    rutas_sin_autenticacion = [
        'index', 'static', 'login', 'login_external',
        'get_equipos', 'actualizar_buque', 'fua_buque', 'fua_post',
        'api_subgrupos', 'api_sistemas', 'api_subsistemas', 'LSA'
    ]
    ...
    if 'buqueId' in request.args:
        ...
        # Si el token no existe, generar uno nuevo
        if not token:
    ...
    # Asignar id_equipo_info de la sesión si está disponible
...
def lsa_view(nombre_buque):
    ...
    # Aseguramos guardar nombre_buque en sesión por si lo necesitas en otras partes
    session['nombre_buque'] = nombre_buque
...
def actualizar_buque():
    ...
    nombre_buque = data.get('nombre_buque', '')
    misiones = data.get('misiones', [])
    datos_puerto_base = data.get('datos_puerto_base', {})

    if not buque_id:
        return jsonify({'error': 'Falta el buque_id'}), 400

    try:
        guardar_o_actualizar_buque(buque_id, nombre_buque, misiones, datos_puerto_base)
        return jsonify({'message': 'Buque actualizado y datos de puerto base almacenados correctamente en la base de datos'}), 200
    except Exception as e:
        app.logger.error(f"Error al actualizar buque: {e}")
        return jsonify({'error': str(e)}), 500
...
def fua_post():
    ...
    nombre_buque = data.get('nombreBuque')
    sistemas = data.get('sistemas')  # 👈 aquí lo recibes
    app.logger.info(nombre_buque)
    app.logger.info(sistemas)

    # Limpia sesión previa
    session.pop('desde_ils', None)

    # Guarda en sesión para usar en la vista final
    session['buque_id'] = buque_id
    session['equipo_id'] = equipo_id
    session['nombre_buque'] = nombre_buque
    session['sistemas'] = sistemas  # 👈 lo guardas para uso posterior

    redirect_url = f"http://localhost:5000/FUA/{nombre_buque}"
...
def fua_buque(nombre_buque):
    ...
    sistemas = session.get('sistemas', [])  # 👈 sistemas cargados desde Laravel
...
def guardar_equipos():
    try:
        ...
        for equipo in equipos:
            nombre = equipo.get("nombre_equipo", "Sin nombre")
            marca = equipo.get("marca", "Genérica")
            modelo = equipo.get("modelo", "N/A")
            descripcion = equipo.get("descripcion", "Sin descripción")
            dimensiones = equipo.get("dimensiones") or "Desconocido"
            ...
            # Datos por defecto
            fecha = "1970-01-01"
            fiabilidad_equipo = 0.0
            GRES = 0
            criticidad_equipo = 1
            id_personal = 1
            id_diagrama = None
            id_procedimiento = None
            id_equipo = 1  # Puedes eliminar este si no lo usas en la función

            cj = generar_codigo_jerarquico(id_subsistema, id_buque)

            # Llamar a la función del database.py
            insertar_equipo_info_api(
                nombre, fecha, fiabilidad_equipo, GRES, criticidad_equipo,
        ...
        return jsonify({"mensaje": "Equipos guardados correctamente"}), 200
...
def equipos_buque(nombre_buque):
    ...
    # Guardar en sesión si se pasó por parámetro
    if buque_id_param:
    ...
    # 🧹 Limpiar desde_ils si venía desde ILS
    session.pop('desde_ils', None)

    # Redirigir sin mostrar `from` ni `buque_id` en la URL
    if from_param or buque_id_param:
        session['from_param'] = from_param  # puede ser None
        return redirect(url_for('equipos_buque', nombre_buque=nombre_buque))

    # Recuperar desde sesión
    from_param = session.pop('from_param', None)
    buque_id = session.get('buque_id')  # persistente hasta que se cambie manualmente

    app.logger.info(f"🔍 [equipos_buque] from_param: {from_param}")
    app.logger.info(f"📦 buque_id desde sesión: {buque_id}")
...
def api_equipo_detalle(id_equipo_info):
    ...
    analisis_funcionales, componentes = obtener_analisis_funcionales_por_equipo_info(id_equipo_info)
    app.logger.info("✅ analisis_funcionales y componentes obtenidos")

    equipo = obtener_informacion_equipo_info(id_equipo_info)
    app.logger.info(f"✅ equipo info obtenido")

    if equipo.get('FUA_FR'):
        datosFUA = json.loads(equipo['FUA_FR'])
    else:
        datosFUA = {}
    app.logger.info("✅ datosFUA listo")

    equipo['AOR_porcentual'] = round(8760 / equipo['AOR'], 2) if equipo.get('AOR') else 0
    app.logger.info("✅ AOR_porcentual calculado")

    fmeas = obtener_fmeas_por_equipo_info(id_equipo_info)
    app.logger.info("✅ fmeas obtenido")

    mtas = obtener_mta_por_equipo_info(id_equipo_info)
    app.logger.info("✅ mtas obtenidas")

    rcms = obtener_rcm_por_equipo_info(id_equipo_info)
    app.logger.info("✅ rcms obtenidos")

    repuestos = obtener_repuestos_por_equipo_info(id_equipo_info)
    app.logger.info("✅ repuestos obtenidos")

    herramientas_relacionadas = obtener_herramientas_relacionadas_por_equipo(id_equipo_info)
    app.logger.info("✅ herramientas_relacionadas obtenidas")

    analisis = list(obtener_analisis_herramientas_por_equipo(id_equipo_info) or [])
    app.logger.info(analisis)

    herramientas = list(obtener_herramientas_especiales_por_equipo(id_equipo_info) or [])
    app.logger.info("✅ herramientas especiales obtenidas")
    ...
    for relacion in herramientas_relacionadas:
        ...
        if id_clase_herramienta == 1:
            ...
            if herramienta_especial:
                herramientas.append(herramienta_especial)
    app.logger.info("✅ herramientas relacionadas procesadas")

    grupo_constructivo = obtener_grupo_constructivo_por_sistema_id(equipo['id_sistema']) if equipo.get('id_sistema') else None
    app.logger.info("✅ grupo_constructivo obtenido")

    subgrupo_constructivo = obtener_subgrupo_constructivo_por_sistema_id(equipo['id_sistema']) if equipo.get('id_sistema') else None
    app.logger.info("✅ subgrupo_constructivo obtenido")
    ...
    procedimiento = obtener_procedimiento_por_id(equipo['id_procedimiento']) if equipo['id_procedimiento'] else None
    app.logger.info("✅ procedimiento obtenido")

    responsable = obtener_personal_por_id(equipo['id_personal']) if equipo['id_personal'] else None
    app.logger.info("✅ responsable obtenido")

    sistema = obtener_sistema_por_id(equipo['id_sistema']) if equipo.get('id_sistema') else None
    app.logger.info("✅ sistema obtenido")

    subsistema = obtener_subsistema_por_id(equipo['id_subsistema']) if equipo.get('id_subsistema') else None
    app.logger.info("✅ subsistema obtenido")

    datos_equipo = obtener_datos_equipo_por_id(equipo['id_equipo']) if equipo.get('id_equipo') else None
    app.logger.info("✅ datos_equipo obtenido")
    ...
    if equipo.get('imagen') and isinstance(equipo['imagen'], bytes):
        ...
        equipo['imagen'] = None
    app.logger.info("✅ imagen procesada")

    tipo_equipo = None
    if datos_equipo and datos_equipo.get('id_tipos_equipos'):
        tipo_equipo = obtener_tipo_equipo_por_id(datos_equipo['id_tipos_equipos'])
    app.logger.info("✅ tipo_equipo obtenido")

    equipo = limpiar_bytes_dict(equipo)
    app.logger.info("✅ equipo limpiado de bytes")

    # Si llegamos aquí, todo está bien antes de jsonify
    app.logger.info("✅ Listo para jsonify")
...
def limpiar_bytes_dict(d):
    ...
    for k, v in d.items():
        if isinstance(v, bytes):
            ...
            if k in ['imagen', 'dibujo_seccion_transversal']:
