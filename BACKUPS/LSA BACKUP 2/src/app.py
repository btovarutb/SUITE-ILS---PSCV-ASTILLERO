...
from src.database import (
    ...
    obtener_equipos_por_buque_completo,
...
def registro_generalidades():
    ...
    if request.method == 'POST':
        try:
            ...
            GRES = int(mec.split()[-1]) if mec.split()[-1].isdigit() else 1  # Por defecto 1
            ...
            cj = request.form.get('codigo_jerarquico')
            cj = cj.strip() if cj else "N/A"
            ...
            id_buque = buque_id or 1  # Por defecto 1 si no se encuentra
            id_sistema_ils = sistema_id_ils or 1  # Por defecto 1 si no se encuentra
            ...
            insertar_equipo_info(
                ...
                id_personal, id_diagrama, id_procedimiento, id_sistema, id_equipo, id_sistema_ils, id_buque, cj
            )
...
def before_request():
    ...
    rutas_sin_autenticacion = ['index', 'static', 'login', 'login_external', 'get_equipos', 'actualizar_buque', 'fua_buque', 'fua_post']
...
def crear_RCM(fmea_id,id_equipo_info):
    ...
    if fmea:
        return render_template('registro_rcm.html',id_equipo_info=id_equipo_info, fmea=fmea, acciones=acciones, editar = False, nombre_equipo=nombre_equipo)
...
def get_equipos(id_buque):
    equipos = obtener_equipos_por_buque(id_buque)
    return jsonify(equipos)
...
def fua_post():
    ...
    # Puedes si quieres guardar datos en sesión para usarlos en la vista final:
    session['buque_id'] = buque_id
    session['equipo_id'] = equipo_id

    # Generas la URL para redirigir al usuario:
...
def fua_buque(nombre_buque):
    ...
    # Ahora sí, obtienes los datos que necesitas
    equipos = obtener_equipos_por_buque_completo(buque_id)
    equipo_seleccionado = next((e for e in equipos if e['id'] == equipo_id), None)

    return render_template('fua_buque.html', buque_id=buque_id, equipos=equipos, nombre_buque=nombre_buque, equipo_seleccionado=equipo_seleccionado)
