...
@retry_on_disconnect 
def insertar_equipo_info(nombre_equipo, fecha, fiabilidad_equipo, GRES, criticidad_equipo,
    ...
    logger.info(f"llego a insertar equipo_info {id_buque},{id_sistema_ils} ")
    try:
        cursor = db.connection.cursor()
        ...
        logger.info(f"Values: ({nombre_equipo}, {fecha}, {fiabilidad_equipo}, {GRES}, {criticidad_equipo}, "
            f"{marca}, {modelo}, {peso_seco}, {dimensiones}, {descripcion}, {imagen_equipo_file}, "
            f"{id_personal}, {id_diagrama}, {id_procedimiento}, {id_sistema}, {id_equipo}, {id_sistema_ils}, {id_buque}, {cj})")
...
def actualizar_equipo_info(id_equipo_info, data):
    try:
        ...
        params = [
            data['nombre_equipo'], data['fecha'], data['fiabilidad_equipo'],
            ...
            data['id_procedimiento'], data['sistema'], data['equipo'], data['cj']
        ]
...
def obtener_rcm_por_equipo_info(id_equipo_info):
    ...
    query = """
        SELECT
            ...
            r.intervalo_inicial_horas,
            r.patron_de_falla,     -- Nuevo campo (Base64 JSON)
            r.tarea_contemplada,   -- Nuevo campo (Texto)
            r.fuente               -- Nuevo campo (Texto)
        FROM rcm r
        LEFT JOIN fmea f ON r.id_fmea = f.id
        LEFT JOIN subsistemas s ON f.id_sistema = s.id
        LEFT JOIN falla_funcional ff ON f.id_falla_funcional = ff.id
        ...
        LEFT JOIN lora_mta l ON m.id = l.id_mta
        LEFT JOIN ocurrencia o ON f.id_ocurrencia = o.id -- Se eliminó la coma extra
        WHERE f.id_equipo_info = %s 
        AND r.estado = 'activo';
    """
    ...
    # Convertir patron_de_falla de JSON a objeto Python
    for rcm in rcms:
        if rcm['patron_de_falla']:
            try:
                rcm['patron_de_falla'] = json.loads(rcm['patron_de_falla'])
            except json.JSONDecodeError:
                rcm['patron_de_falla'] = None  # Manejo de error en caso de datos inválidos
...
@retry_on_disconnect
def obtener_equipos_por_buque(id_buque):
    cursor = db.connection.cursor(MySQLdb.cursors.DictCursor)
    query = """
        SELECT id, nombre_equipo, id_sistema_ils, AOR FROM equipo_info 
        WHERE id_buque = %s
    """
...
@retry_on_disconnect
def guardar_o_actualizar_buque(buque_id, nombre_buque, misiones, datos_puerto_base=None):
    """
    Guarda o actualiza las misiones y datos de puerto base de un buque en la base de datos.
    """
    cursor = db.connection.cursor()
    ...
    if resultado:
        query_update = """
            UPDATE buques_info
            SET nombre_buque = %s, misiones = %s, datos_puerto_base = %s
            WHERE buque_id = %s
        """
        cursor.execute(query_update, (nombre_buque, json.dumps(misiones), json.dumps(datos_puerto_base), buque_id))
    else:
        query_insert = """
            INSERT INTO buques_info (buque_id, nombre_buque, misiones, datos_puerto_base)
            VALUES (%s, %s, %s, %s)
        """
        cursor.execute(query_insert, (buque_id, nombre_buque, json.dumps(misiones), json.dumps(datos_puerto_base)))
...
def obtener_datos_buque(buque_id):
    ...
    try:
        query = "SELECT misiones, datos_puerto_base FROM buques_info WHERE buque_id = %s"
...
@retry_on_disconnect
def obtener_equipos_por_buque_completo(id_buque):
    cursor = db.connection.cursor(MySQLdb.cursors.DictCursor)
    query = """
        SELECT * FROM equipo_info 
        WHERE id_buque = %s
    """
    cursor.execute(query, (id_buque,))  # la coma es clave aquí
    equipos = cursor.fetchall()
    cursor.close()
    return equipos
